...
def create_pdf(cid, data):
    ...
    for label, value in fields:
        ...
        pdf.cell(0, 9, clean_text(value), 0, 1)
    ...
    pdf.set_font('Arial', '', 11)
    pdf.set_x(15)
    pdf.multi_cell(180, 8, clean_text(data['description']))
    ...
    pdf.cell(0, 8, 'Confidential Document - Public Complaint System', 0, 1, 'C')
...
def download_link(pdf_bytes, filename):
    try:
        ...
        b64 = base64.b64encode(pdf_bytes.encode('latin-1', errors='replace')).decode()
        ...
        if complaints:
            ...
            for cid, data in reversed(list(complaints.items())):
                ...
                status_info = {
                    ...
                    "In Progress": ("IN PROGRESS", "#2196F3"),
                    "Resolved": ("RESOLVED", "#4CAF50"),
                    "Rejected": ("REJECTED", "#f44336")
                }
                status_text, color = status_info.get(data['status'], ("UNKNOWN", "#999"))
                
                with st.expander(f"[{status_text}] {data['subject']} | {data['name']} | {data['date']}"):
